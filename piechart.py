...
# Create the pie chart for Total People Killed with a discrete color scale
fig_killed = go.Figure(data=[go.Pie(labels=labels_killed, 
                                    values=killed_data, 
                                    hole=0.3, 
                                    title="Total People Killed - Breakdown by Category",
                                    marker=dict(colors=['#440154', '#482878', '#3E4A89', '#2D708E']))])  # Custom discrete colors

# Create the pie chart for Total People Injured with a discrete color scale
fig_injured = go.Figure(data=[go.Pie(labels=labels_injured, 
                                     values=injured_data, 
                                     hole=0.3, 
                                     title="Total People Injured - Breakdown by Category",
                                     marker=dict(colors=['#440154', '#482878', '#3E4A89', '#2D708E']))])  # Custom discrete colors
